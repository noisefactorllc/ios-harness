...
function makeDriver(browser, { udid, mode = 'web' } = {}) {
    ...
    const d = {
        ...
        async click(sel, { timeout = 15000 } = {}) {
            const el = await browser.$(sel)
            await el.waitForExist({ timeout })
            // nativeWebTap taps by SCREEN COORDINATES; on a present-but-hidden element
            // (responsive layouts keep desktop controls in the DOM) it mis-fires at a
            // stale location — which can hit a neighbouring link and wedge the webview.
            // Native-tap only when actually displayed; otherwise dispatch a DOM click,
            // which fires the handler without a coordinate mis-tap.
