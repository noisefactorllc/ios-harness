...
function harnessState() {
    ...
    return {
        ...
        // WebGL-only counts (exclude WebGPU): a live WebGPU context must not mask a
        // dead WebGL renderer, so liveness checks for WebGL products use these.
        glWebglContextCount: H.glContexts.filter(function (g) {
            return g.type !== 'webgpu'
        }).length,
        glWebglLost: H.glContexts.filter(function (g) {
            return g.type !== 'webgpu' && g.lost
        }).length,
...
/**
 * Enumerate media input devices. Used to detect whether a camera is actually
 * available: the iOS Simulator exposes a virtual camera to NATIVE apps (WKWebView)
 * but NOT to Mobile Safari — so camera apps test their real capture/render path in
 * native mode and their graceful no-camera degradation in web mode.
 */
function mediaDeviceKinds(done) {
    try {
        if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) {
            return done({ ok: false, video: 0, audio: 0 })
        }
        navigator.mediaDevices
            ...
            .then(function (ds) {
                ...
                ds.forEach(function (d) {
                    if (d.kind === 'videoinput') v++
                    else if (d.kind === 'audioinput') a++
                })
                done({ ok: true, video: v, audio: a, total: ds.length })
            })
            .catch(function (e) {
                done({ ok: false, video: 0, audio: 0, reason: String((e && e.message) || e) })
            })
    } catch (e) {
        done({ ok: false, video: 0, audio: 0, reason: String((e && e.message) || e) })
    }
