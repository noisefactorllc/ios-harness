...
async function runProduct(product, { appium, device, runDir, log, sharedOnly }) {
    ...
    try {
        ...
        if (product.orientation === 'landscape') {
            const r = await driver.setOrientation('LANDSCAPE')
            log(`    orientation: landscape${r === true ? '' : ` (note: ${JSON.stringify(r)})`}`)
        }
...
async function run({ keys, sharedOnly = false, log = console.log } = {}) {
    const selected = products.resolve(keys)
    ...
    try {
        for (const product of selected) {
            log(`\n▶ ${product.title} (${product.key})${product.backendGated ? '  [front-end only — backend not exercised]' : ''}`)
            byProduct[product.key] = await runProduct(product, { appium, device, runDir, log, sharedOnly })
        }
    } finally {
